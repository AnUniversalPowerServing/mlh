...
"approveNewsFeedUnionLevel":"Y","createMovementBranchLevel":"Y","approveMovementBranchLevel":"Y","updatePerm2":"Y"}}
*/
var ROLE_INFO={};
...
"createMovementBranchLevel":"Y","approveMovementBranchLevel":"Y","updatePerm2":"Y"};
...
function load_createRole_list(){
 ...
 for(var role_Id in ROLE_INFO){
  ...
  var updatePerm2 = roleInfoObject["updatePerm2"];
  ...
  content+='<b>Movements <i class="fa fa-angle-down pull-right" aria-hidden="true"></i></b>';
  content+='</div>';
  ...
  content+='<div class="list-group-item" data-toggle="collapse" data-target="#perm_rolePerm_'+role_Id+'">';
  content+='<b>Update Role Permissions <i class="fa fa-angle-down pull-right" aria-hidden="true"></i></b>';
  content+='</div>';
  content+='<div id="perm_rolePerm_'+role_Id+'" class="collapse">';
  
  content+='<div class="list-group-item">';
  content+='<div class="container-fluid">';
  content+='<div class="row mtop15p">';
  content+='<div class="col-xs-8 pad0"><div class="pad5">Update Role Permissions</div></div>';
  content+='<div class="col-xs-3 pull-right">';
  content+='<input id="input_updateRolePermissions_'+role_Id+'" type="checkbox" data-toggle="toggle" data-on="Yes" ';
  if(updatePerm2==='Y'){ content+='checked '; }
  content+='data-off="No" onchange="javascript:setPerm_role_permissions_update(\''+role_Id+'\');">';
  content+='</div>';
  content+='</div>';
  content+='</div>';
  content+='</div>';
...
function setPerm_role_permissions_update(role_Id){
 var status = $('#input_updateRolePermissions_'+role_Id).prop('checked');
 if(status){ status='Y'; }
 else { status='N'; }
 ROLE_INFO[role_Id].updatePerm2=status;
 console.log(JSON.stringify(ROLE_INFO));
}
/*****************************************************************************************************************************/
/**************************************************** ADD MEMBERS ************************************************************/
...
function displayTemplate_membersContent(member_Id, profile_pic, name, minlocation, location, state, country){
	...
	MEMBER_DISPLAYCONTENT+='</div>'; // container-fluid 
	MEMBER_DISPLAYCONTENT+='</div>'; // list-group-item
	MEMBER_DISPLAYCONTENT+='</div>'; // splPerm_movement_collapse
	
	MEMBER_DISPLAYCONTENT+='<div class="list-group-item" data-toggle="collapse" ';
	MEMBER_DISPLAYCONTENT+='data-target="#splPerm_updateSplPerm_'+member_Id+'">';
    MEMBER_DISPLAYCONTENT+='<b>Update Special Permission <i class="fa fa-angle-down pull-right" aria-hidden="true"></i></b>';
    MEMBER_DISPLAYCONTENT+='</div>';
	
	MEMBER_DISPLAYCONTENT+='<div id="splPerm_updateSplPerm_'+member_Id+'" class="collapse">'; // splPerm_movement_
	MEMBER_DISPLAYCONTENT+='<div class="list-group-item">'; // list-group-item
	MEMBER_DISPLAYCONTENT+='<div class="container-fluid">'; // container-fluid

	MEMBER_DISPLAYCONTENT+='<div class="row mtop15p">';
    MEMBER_DISPLAYCONTENT+='<div class="col-xs-8 pad0"><div class="pad5">Update Special Permission by this Member</div></div> ';
    MEMBER_DISPLAYCONTENT+='<div class="col-xs-3 pull-right">';
    MEMBER_DISPLAYCONTENT+='<input id="input_updateSplPerm_'+member_Id+'" type="checkbox" data-toggle="toggle" ';
	MEMBER_DISPLAYCONTENT+='data-on="Yes" data-off="No" ';
    MEMBER_DISPLAYCONTENT+='onchange="javascript:setPerm_splPermissions_members(\''+member_Id+'\');">';
    MEMBER_DISPLAYCONTENT+='</div>';
    MEMBER_DISPLAYCONTENT+='</div>';
...
function addToArray_members(){
 ...
 if(MEMBERS_REQ_LIST[MEMBER_ID]===undefined){
   ...
   createMovementDomainLevel:'N',  approveMovementDomainLevel:'N', updatePerm1:'N' };
...
function setPerm_splPermissions_members(member_Id){
 var status = $('#input_updateSplPerm_'+member_Id).prop('checked');
 if(status){ status='Y'; }
 else { status='N'; }
 MEMBERS_REQ_LIST[member_Id].updateSplPermissions=status;
 console.log(JSON.stringify(MEMBERS_REQ_LIST));
}
 ...
 if(!finished){
  ...
  js_ajax('GET',PROJECT_URL+'backend/php/dac/controller.module.app.community.professional.php',
   { action:'CREATE_PROFESSIONALCOMMUNITY', union_Id: UNION_ID, unionName: UNION_NAME, domain_Id: DOMAIN_ID, 
	 ...
	 members: membersList, admin_Id: AUTH_USER_ID },function(response){
	hide_toggleMLHLoader('body');
