...
/* CLASS DESCRIPTION : This DAL Class is used by Controller (controller.module.app.user.authentication.php) */
  ...
  /* PAGES UTILIZED : auth-part-02.php */
	...
	$query02.="(SELECT CONCAT('[',GROUP_CONCAT('{\"mcountrycode\":\"',user_contacts.mcountrycode,'\",\"mobile\":\"',";
	$query02.="user_contacts.mobile,'\"}'),']') FROM user_contacts WHERE user_contacts.user_Id=(".$query01.")) As ";
	$query02.="contactData FROM user_account WHERE user_account.user_Id=(".$query01.");";
    return $query02;  
  }
  ...
  function query_updateUserInfo($user_Id,$username,$surName,$name,$dob,$gender,
	    $profile_pic,$about_me,$minlocation,$location,$state,$country,$user_tz,$acc_active){
  ...
  function query_updateUserContactInfo($user_Id,$initial_mcountrycode,$initial_mobile,$changed_mcountrycode,$changed_mobile){
    $sql="UPDATE user_contacts SET mcountrycode='".$changed_mcountrycode."', mobile='".$changed_mobile."', mob_val='Y' ";
	$sql.="WHERE user_Id='".$user_Id."' AND mcountrycode='".$initial_mcountrycode."' AND mobile='".$initial_mobile."';";
	return $sql;
  }
  ...
  function query_validate_userPhoneNumber($mcountrycode,$mobile){
   $sql="SELECT count(*) FROM user_contacts WHERE mcountrycode='".$mcountrycode."' AND mobile='".$mobile."';";
   return $sql;
  }
